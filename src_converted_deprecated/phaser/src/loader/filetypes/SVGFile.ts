 ...
 */
export class SVGFile extends File {

    constructor(loader: any, key: string | Record<string, any>, url?: string, svgConfig?: SVGSizeConfig, xhrSettings?: any)
    {
    ...
    onProcess(): void
    {
        ...
        const text = this.xhrLoader.responseText;
        let svg: string[] = [ text ];
        ...
        resize: if (width && height || scale)
        {
            ...
            const svgXML = xml.getElementsByTagName('svg')[0];

            const hasViewBox = svgXML.hasAttribute('viewBox');
            let svgWidth = parseFloat(svgXML.getAttribute('width')!);
            let svgHeight = parseFloat(svgXML.getAttribute('height')!);
 ...
 */
FileTypesManager.register('svg', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, url?: string, svgConfig?: SVGSizeConfig, xhrSettings?: any): any
{
