 ...
 */
export class UnityAtlasFile extends MultiFile {

    constructor(loader: any, key: string | Record<string, any>, textureURL?: string | string[], atlasURL?: string, textureXhrSettings?: any, atlasXhrSettings?: any)
    {
        let image: ImageFile;
        let data: TextFile;
        ...
        else
        {
            ...
            data = new TextFile(loader, key as string, atlasURL, atlasXhrSettings);
        }

        if (image.linkFile)
        {
            //  Image has a normal map
            super(loader, 'unityatlas', key as string, [ image, data, image.linkFile ]);
        }
        else
        {
            super(loader, 'unityatlas', key as string, [ image, data ]);
        }
    }
    ...
    addToCache(): void
    {
        if (this.isReadyToProcess())
        {
            ...
            const text = this.files[1];
            const normalMap = (this.files[2]) ? this.files[2].data : null;
 ...
 */
FileTypesManager.register('unityAtlas', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, textureURL?: string | string[], atlasURL?: string, textureXhrSettings?: any, atlasXhrSettings?: any): any
{
    let multifile: UnityAtlasFile;
