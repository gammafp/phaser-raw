 ...
 */
export class AudioSpriteFile extends MultiFile {

    constructor(loader: any, key: string | Record<string, any>, jsonURL?: string | Record<string, any>, audioURL?: string | string[], audioConfig?: any, audioXhrSettings?: any, jsonXhrSettings?: any)
    {
        ...
        if (!audioURL)
        {
            data = new JSONFile(loader, key as string, jsonURL, jsonXhrSettings);

            super(loader, 'audiosprite', key as string, [ data ]);
        ...
        {
            const audio = AudioFile.create(loader, key, audioURL, audioConfig, audioXhrSettings);

            if (audio)
            {
                data = new JSONFile(loader, key as string, jsonURL, jsonXhrSettings);

                super(loader, 'audiosprite', key as string, [ audio, data ]);
    ...
    onFileComplete(file: File): void
    {
        ...
        if (index !== -1)
        {
            ...
            if (this.config.resourceLoad && file.type === 'json' && file.data.hasOwnProperty('resources'))
            {
                ...
                const urls = file.data.resources;

                const audioConfig = GetFastValue(this.config, 'audioConfig');
                const audioXhrSettings = GetFastValue(this.config, 'audioXhrSettings');

                const audio = AudioFile.create(this.loader, file.key, urls, audioConfig, audioXhrSettings);
    ...
    addToCache(): void
    {
        ...
        {
            const fileA = this.files[0];
            const fileB = this.files[1];
 ...
 */
FileTypesManager.register('audioSprite', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, jsonURL?: string | Record<string, any>, audioURL?: string | string[], audioConfig?: any, audioXhrSettings?: any, jsonXhrSettings?: any): any
{
    const game = this.systems.game;
    const gameAudioConfig = game.config.audio;
    ...
    let multifile: AudioSpriteFile | undefined;
