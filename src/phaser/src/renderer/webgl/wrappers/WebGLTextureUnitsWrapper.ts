 ...
 */
export class WebGLTextureUnitsWrapper {
    ...
    init(): void
    {
        ...
        for (let unit = this.renderer.maxTextures - 1; unit >= 0; unit--)
        {
     ...
     */
    bind(texture: any | null, unit: number, force?: boolean, forceActive?: boolean): void
    {
        const needsBind = this.units[unit] !== texture;
        ...
        const glTexture = texture ? texture.webGLTexture : null;
        const gl = this.renderer.gl;
     ...
     */
    bindUnits(textures: (any | null | undefined)[], force?: boolean): void
    {
        const length = Math.min(textures.length, this.renderer.maxTextures);
