...
import * as ScaleEvents from '../../scale/events';
import * as TextureEvents from '../../textures/events';
import { NOOP } from '../../utils/NOOP';
import { DrawingContext } from './DrawingContext';
import { Utils } from './Utils';
import { WebGLSnapshot } from '../snapshot/WebGLSnapshot';
import { WebGLBufferWrapper } from './wrappers/WebGLBufferWrapper';
import { WebGLGlobalWrapper } from './wrappers/WebGLGlobalWrapper';
import { WebGLProgramWrapper } from './wrappers/WebGLProgramWrapper';
import { WebGLShaderSetterWrapper } from './wrappers/WebGLShaderSetterWrapper';
import { WebGLTextureWrapper } from './wrappers/WebGLTextureWrapper';
import { WebGLTextureUnitsWrapper } from './wrappers/WebGLTextureUnitsWrapper';
import { WebGLFramebufferWrapper } from './wrappers/WebGLFramebufferWrapper';
import { WebGLVAOWrapper } from './wrappers/WebGLVAOWrapper';
import { WebGLBlendParametersFactory } from './parameters/WebGLBlendParametersFactory';
import { WebGLGlobalParametersFactory } from './parameters/WebGLGlobalParametersFactory';
import { RenderNodeManager } from './renderNodes/RenderNodeManager';
...
{
    const SPECTOR = require('phaser3spectorjs');
    DEBUG = true;
}
...
export class WebGLRenderer extends EventEmitter {
    ...
    init(config: any)
    {
        ...
        const clearColor = config.backgroundColor;
        ...
        //  Mipmaps
        const validMipMaps = [ 'NEAREST', 'LINEAR', 'NEAREST_MIPMAP_NEAREST', 'LINEAR_MIPMAP_NEAREST', 'NEAREST_MIPMAP_LINEAR', 'LINEAR_MIPMAP_LINEAR' ];
    ...
    setExtensions()
    {
        ...
        const angleString = 'ANGLE_instanced_arrays';
        ...
        {
            const parallelShaderCompileString = 'KHR_parallel_shader_compile';
     ...
     */
    setContextHandlers(contextLost?: () => void, contextRestored?: () => void)
    {
     ...
     */
    dispatchContextRestored(event: any)
    {
        const gl = this.gl;
        ...
        const wrapperCreateResource = function (wrapper: any)
        {
     ...
     */
    captureFrame(quickCapture?: boolean, fullCapture?: boolean)
    {
    ...
    log()
    {
        if (DEBUG && this.spector)
        {
            const t = Array.prototype.slice.call(arguments).join(' ');
     ...
     * @param {boolean} [fullCapture=false] - If `true` all details are captured.
     */
    startCapture(commandCount?: number, quickCapture?: boolean, fullCapture?: boolean)
    {
    ...
    onCapture(capture: any)
    {
        if (DEBUG)
        {
            const view = this.spector.getResultUI();
     ...
     */
    onResize(gameSize: any, baseSize: any)
    {
    ...
    getCompressedTextures()
    {
        ...
        const extEXTString = 'EXT_texture_compression_';

        const hasExt = function (gl: WebGLRenderingContext, format: string): any
        {
            const results = gl.getExtension(extString + format) || gl.getExtension(wkExtString + format) || gl.getExtension(extEXTString + format);

            if (results)
            {
                const glEnums: Record<number, string> = {};
     ...
     */
    getCompressedTextureName(baseFormat: string, format?: number)
    {
     ...
     */
    supportsCompressedTexture(baseFormat: string, format?: number)
    {
        const supportedFormats = this.compression[baseFormat.toUpperCase()];
     ...
     * @return {this} This WebGLRenderer instance.
     */
    setProjectionMatrix(width: number, height: number, flipY?: boolean)
    {
     ...
     * @return {this} This WebGLRenderer instance.
     */
    setProjectionMatrixFromDrawingContext(drawingContext: any)
    {
     ...
     */
    addBlendMode(func: number[], equation: number)
    {
        const index = this.blendModes.push(WebGLBlendParametersFactory.createCombined(this, true, undefined, equation, func[0], func[1])) - 1;
     ...
     * @return {this} This WebGLRenderer instance.
     */
    updateBlendMode(index: number, func: number[], equation: number | number[])
    {
        if (index > 17 && this.blendModes[index])
        {
            const blendMode = this.blendModes[index];
            if (func.length === 2)
            {
     ...
     */
    clearFramebuffer(color?: number[], stencil?: number, depth?: number)
    {
        const gl = this.gl;
        let bits = 0;
        if (color)
     ...
     */
    createTextureFromSource(source: any, width: number, height: number, scaleMode: number, forceClamp?: boolean, flipY?: boolean)
    {
        ...
        const pow = IsSizePowerOfTwo(width, height);
        ...
        {
            const isCompressed = source && source.compressed;
            const isMip = (!isCompressed && pow) || (isCompressed && source.mipmaps.length > 1);
     ...
     */
    createTexture2D(mipLevel: number, minFilter: number, magFilter: number, wrapT: number, wrapS: number, format: number, pixels: any, width?: number, height?: number, pma?: boolean, forceSize?: boolean, flipY?: boolean)
    {
        if (typeof width !== 'number') { width = pixels ? pixels.width : 1; }
        if (typeof height !== 'number') { height = pixels ? pixels.height : 1; }

        const texture = new WebGLTextureWrapper(this, mipLevel, minFilter, magFilter, wrapT, wrapS, format, pixels, width, height, pma, forceSize, flipY);
     ...
     */
    createFramebuffer(renderTexture: any, addStencilBuffer?: boolean, addDepthBuffer?: boolean)
    {
        if (!Array.isArray(renderTexture) && renderTexture !== null)
        {
            renderTexture = [ renderTexture ];
        }
        const framebuffer = new WebGLFramebufferWrapper(
            this,
            renderTexture,
            addStencilBuffer,
            addDepthBuffer
    ...
    {
        const wrapper = new WebGLProgramWrapper(this, vertexShader, fragmentShader);
     ...
     */
    createVertexBuffer(initialDataOrSize: ArrayBuffer | number, bufferUsage: number)
    {
        const gl = this.gl;
        const vertexBuffer = new WebGLBufferWrapper(this, initialDataOrSize, gl.ARRAY_BUFFER, bufferUsage);
     ...
     */
    createIndexBuffer(initialDataOrSize: ArrayBuffer | number, bufferUsage: number)
    {
        const gl = this.gl;
        const indexBuffer = new WebGLBufferWrapper(this, initialDataOrSize, gl.ELEMENT_ARRAY_BUFFER, bufferUsage);
     ...
     */
    createVAO(program: any, indexBuffer: any, attributeBufferLayouts: any[])
    {
        const vao = new WebGLVAOWrapper(this, program, indexBuffer, attributeBufferLayouts);
    ...
    preRender()
    {
        ...
        const baseDrawingContext = this.baseDrawingContext;
     ...
     */
    render(scene: any, children: any[], camera: any)
    {
        if (this.contextLost) { return; }
     ...
     */
    drawElements(drawingContext: any, textures: any[], program: any, vao: any, count: number, offset: number, topology?: number)
    {
        const gl = this.gl;
     ...
     */
    drawInstancedArrays(drawingContext: any, textures: any[], program: any, vao: any, first: number, count: number, instanceCount: number, topology?: number)
    {
        const gl = this.gl;
     ...
     */
    snapshot(callback: any, type?: string, encoderOptions?: number)
    {
     ...
     * @return {this} This WebGL Renderer.
     */
    snapshotArea(x: number, y: number, width: number, height: number, callback: any, type?: string, encoderOptions?: number)
    {
        const state = this.snapshotState;
     ...
     * @return {this} This WebGL Renderer.
     */
    snapshotPixel(x: number, y: number, callback: any)
    {
     ...
     * @return {this} This WebGL Renderer.
     */
    snapshotFramebuffer(framebuffer: any, bufferWidth: number, bufferHeight: number, callback: any, getPixel?: boolean, x?: number, y?: number, width?: number, height?: number, type?: string, encoderOptions?: number)
    {
     ...
     */
    canvasToTexture(srcCanvas: HTMLCanvasElement, dstTexture?: any, noRepeat?: boolean, flipY?: boolean)
    {
     ...
     */
    createCanvasTexture(srcCanvas: HTMLCanvasElement, noRepeat?: boolean, flipY?: boolean)
    {
        if (noRepeat === undefined) { noRepeat = false; }
        if (flipY === undefined) { flipY = true; }
     ...
     */
    updateCanvasTexture(srcCanvas: HTMLCanvasElement, dstTexture: any, flipY?: boolean, noRepeat?: boolean)
    {
        if (flipY === undefined) { flipY = true; }
        if (noRepeat === undefined) { noRepeat = false; }
     ...
     * @return {Phaser.Renderer.WebGL.Wrappers.WebGLTextureWrapper} The newly created, or updated, WebGLTextureWrapper.
     */
    videoToTexture(srcVideo: HTMLVideoElement, dstTexture?: any, noRepeat?: boolean, flipY?: boolean)
    {
        if (noRepeat === undefined) { noRepeat = false; }
        if (flipY === undefined) { flipY = true; }
     ...
     * @return {Phaser.Renderer.WebGL.Wrappers.WebGLTextureWrapper} The newly created WebGLTextureWrapper.
     */
    createVideoTexture(srcVideo: HTMLVideoElement, noRepeat?: boolean, flipY?: boolean)
    {
        if (noRepeat === undefined) { noRepeat = false; }
        if (flipY === undefined) { flipY = true; }
     ...
     * @return {Phaser.Renderer.WebGL.Wrappers.WebGLTextureWrapper} The updated WebGLTextureWrapper. This is the same wrapper object as `dstTexture`.
     */
    updateVideoTexture(srcVideo: HTMLVideoElement, dstTexture: any, flipY?: boolean, noRepeat?: boolean)
    {
        if (flipY === undefined) { flipY = true; }
        if (noRepeat === undefined) { noRepeat = false; }
     ...
     * @return {Phaser.Renderer.WebGL.Wrappers.WebGLTextureWrapper} The newly created WebGLTextureWrapper.
     */
    createUint8ArrayTexture(data: Uint8Array, width: number, height: number, pma?: boolean, flipY?: boolean)
    {
        const gl = this.gl;
     ...
     */
    setTextureFilter(texture: any, filter: number)
    {
        const gl = this.gl;
        const glFilter = (filter === 0) ? gl.LINEAR : gl.NEAREST;

        const texUnits = this.glTextureUnits;
        const currentTexture = texUnits.units[0];
     ...
     * @return {this} This WebGL Renderer instance.
     */
    setTextureWrap(glTextureWrapper: any, wrapModeS: number, wrapModeT: number)
    {
        const gl = this.gl;
    ...
    destroy()
    {
        ...
        const wrapperDestroy = function (wrapper: any)
        {
            wrapper.destroy();
        };
