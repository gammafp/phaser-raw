 ...
 */
export class SubmitterQuad extends RenderNode {

    batchHandler: string;
    _renderOptions: any;
    _lightingOptions: any;
    _tempMatrix: any;
    _tempMatrix2: any;
    ...
    {
        config = Merge(config || {}, SubmitterQuad.defaultConfig);
    ...
    setRenderOptions(gameObject: any, normalMap?: any, normalMapRotation?: number): void
    {
        ...
        if (gameObject.lighting)
        {
            ...
            if (isNaN(normalMapRotation!))
            {
                ...
                {
                    const matrix = gameObject.getWorldTransformMatrix(this._tempMatrix, this._tempMatrix2);
                    normalMapRotation = matrix.rotationNormalized;
                }
            }

            // Get self-shadow.
            const selfShadow = gameObject.selfShadow;
            let selfShadowEnabled = selfShadow.enabled;
