...
const copyPoints = function (p: { x: number; y: number }): { x: number; y: number } { return { x: p.x, y: p.y }; };

const commonObjectProps = [ 'id', 'name', 'type', 'rotation', 'properties', 'visible', 'x', 'y', 'width', 'height' ];
 ...
 */
export const ParseObject = function (tiledObject: any, offsetX?: number, offsetY?: number): any
{
    if (offsetX === undefined) { offsetX = 0; }
    if (offsetY === undefined) { offsetY = 0; }

    const parsedObject = Pick(tiledObject, commonObjectProps);
