 ...
 */
export const ParseCSV = (name: string, data: string, tileWidth: number, tileHeight: number, insertNull?: boolean): MapData =>
{
    const array2D: any[][] = data
        .trim()
        .split('\n')
        .map((row: string) => row.split(','));

    const map = Parse2DArray(name, array2D, tileWidth, tileHeight, insertNull);
