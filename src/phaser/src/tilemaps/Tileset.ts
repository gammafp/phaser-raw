...
export class Tileset {
    ...
    columns: number;
    total: number;
    texCoordinates: Array<{ x: number; y: number }>;
    animationSearchThreshold: number;
    maxAnimationLength: number;
    _animationDataTexture: any;
    _animationDataIndexMap: Map<number, number> | null;
    ...
    getAnimatedTileId(tileIndex: number, milliseconds: number): number | null
    {
        ...
        const animData = this.getTileData(tileIndex);
    ...
    setImage(texture: any): this
    {
        ...
        const bounds = texture.getFrameBounds();
     ...
     * @return {Phaser.Tilemaps.Tileset} This Tileset object.
     */
    setSpacing(margin?: number, spacing?: number): this
    {
     ...
     * @return {Phaser.Tilemaps.Tileset} This Tileset object.
     */
    updateTileData(imageWidth: number, imageHeight: number, offsetX?: number, offsetY?: number): this
    {
        ...
        let tx = this.tileMargin + offsetX;
        let ty = this.tileMargin + offsetY;
     ...
     */
    getAnimationDataTexture(renderer: any): any
    {
        if (!this._animationDataTexture)
        {
     ...
     */
    getAnimationDataIndexMap(renderer: any): Map<number, number>
    {
     ...
     */
    createAnimationDataTexture(renderer?: any): void
    {
        const tileData = this.tileData;
        const total = this.total;

        const animations: Array<[number, number]> = [];
        const animFrames: Array<[number, number]> = [];
        const indexToAnimMap = new Map<number, number>();

        let maxLength = 0;

        for (let i = 0; i < total; i++)
        {
            const tileDatum = tileData[i];

            if (tileDatum && tileDatum.animation)
            {
                const animation = tileDatum.animation;
                const animationDuration = tileDatum.animationDuration;
        ...
        const totalTuples = animations.length + animFrames.length;
        ...
        const size = totalTuples * 2;
        const width = Math.min(size, 4096);
        const height = Math.ceil(size / 4096);
