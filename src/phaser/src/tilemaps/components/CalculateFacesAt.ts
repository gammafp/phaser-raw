...
export const CalculateFacesAt = (tileX: number, tileY: number, layer: any): void =>
{
    ...
    const below = GetTileAt(tileX, tileY + 1, true, layer);
    const left = GetTileAt(tileX - 1, tileY, true, layer);
    const right = GetTileAt(tileX + 1, tileY, true, layer);
    const tileCollides = tile && tile.collides;
