...
import { PutTileAt } from './PutTileAt';
 ...
 */
export const PutTilesAt = (tilesArray: number[] | number[][] | Tile[] | Tile[][], tileX: number, tileY: number, recalculateFaces: boolean, layer: LayerData): void => {
    ...
    {
        tilesArray = [ tilesArray as any ];
    }

    var height = tilesArray.length;
    var width = (tilesArray[0] as any[]).length;

    for (var ty = 0; ty < height; ty++)
    {
        for (var tx = 0; tx < width; tx++)
        {
            var tile = (tilesArray as any[][])[ty][tx];
