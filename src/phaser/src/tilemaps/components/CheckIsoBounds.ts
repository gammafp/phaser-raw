 ...
 */
export const CheckIsoBounds = (tileX: number, tileY: number, layer: any, camera?: any): boolean =>
{
    const tilemapLayer = layer.tilemapLayer;

    const cullPaddingX = tilemapLayer.cullPaddingX;
    const cullPaddingY = tilemapLayer.cullPaddingY;

    const pos = tilemapLayer.tilemap.tileToWorldXY(tileX, tileY, point, camera, tilemapLayer);
