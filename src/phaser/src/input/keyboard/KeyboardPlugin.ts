 ...
 */
export class KeyboardPlugin extends EventEmitter {
    game: any;
    scene: any;
    settings: any;
    sceneInputPlugin: any;
    manager: any;
    enabled: boolean;
    keys: any[];
    combos: any[];
    ...
    constructor(sceneInputPlugin: any)
    {
        super();
    ...
    boot(): void
    {
        ...
        const captures = GetValue(settings, 'keyboard.capture', null);
    ...
    /**
     * @method Phaser.Input.Keyboard.KeyboardPlugin#addCaptures
     * @since 3.60.0
     *
     * @param {(string|number|number[]|any[])} keycode - The Key Codes to enable event capture for.
     *
     * @return {this} This KeyboardPlugin object.
     */
    addCaptures(keycode: string | number | number[] | any[]): this
    {
        return this.addCapture(keycode);
    }

    /**
     * @method Phaser.Input.Keyboard.KeyboardPlugin#removeCaptures
     * @since 3.60.0
     *
     * @param {(string|number|number[]|any[])} keycode - The Key Codes to disable event capture for.
     *
     * @return {this} This KeyboardPlugin object.
     */
    removeCaptures(keycode: string | number | number[] | any[]): this
    {
        return this.removeCapture(keycode);
    }
     ...
     */
    addKeys(keys: any, enableCapture: boolean = true, emitOnRepeat: boolean = false): any
    {
        const output: any = {};
     ...
     */
    addKey(key: Key | string | number, enableCapture: boolean = true, emitOnRepeat: boolean = false): Key
    {
        ...
        {
            key = KeyCodes[key.toUpperCase() as keyof typeof KeyCodes];
        }
     ...
     * @return {this} This KeyboardPlugin object.
     */
    removeKey(key: Key | string | number, destroy: boolean = false, removeCapture: boolean = false): this
    {
        const keys = this.keys;
        let ref;
        ...
        if (keys[key as number])
        {
            ref = keys[key as number];

            keys[key as number] = undefined;
        }
     ...
     * @return {this} This KeyboardPlugin object.
     */
    removeAllKeys(destroy: boolean = false, removeCapture: boolean = false): this
    {
        const keys = this.keys;
     ...
     */
    createCombo(keys: string | number[] | any[], config?: any): KeyCombo
    {
     ...
     */
    checkDown(key: Key, duration: number = 0): boolean
    {
    ...
    update(): void
    {
        ...
        for (let i = 0; i < len; i++)
        {
            ...
            if (event.type === 'keydown')
            {
                ...
                if (!(event as any).cancelled && (!key || !repeat))
                {
                    if (KeyMap[code])
                    {
