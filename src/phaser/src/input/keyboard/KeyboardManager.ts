...
export class KeyboardManager {
    ...
    startListeners(): void
    {
        ...
        this.onKeyDown = (event: KeyboardEvent): void => {
            ...
            const modified = (event.altKey || event.ctrlKey || event.shiftKey || event.metaKey);
    ...
    addCapture(keycode: string | number | number[] | any[]): void
    {
        ...
        {
            let code: any = keycode[i];

            if (typeof code === 'string')
            {
                code = KeyCodes[code.trim().toUpperCase() as keyof typeof KeyCodes];
            }
    ...
    removeCapture(keycode: string | number | number[] | any[]): void
    {
        ...
        for (let i = 0; i < keycode.length; i++)
        {
            ...
            if (typeof code === 'string')
            {
                code = KeyCodes[code.toUpperCase() as keyof typeof KeyCodes];
            }
