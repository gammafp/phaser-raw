...
export class InputManager {
    ...
    time: number;
    _tempPoint: { x: number; y: number };
    _tempHitTest: any[];
    _tempMatrix: any;
    _tempMatrix2: any;
    _tempSkip: boolean;
    mousePointerContainer: any[];
    ...
    constructor(game: any, config: any)
    {
        ...
        for (let i = 0; i <= this.pointersTotal; i++)
        {
            const pointer = new Pointer(this, i);
     ...
     */
    setCanvasOver(event: MouseEvent | TouchEvent): void
    {
        this.isOver = true;
     ...
     * @param {(MouseEvent|TouchEvent)} event - The DOM Event.
     */
    setCanvasOut(event: MouseEvent | TouchEvent): void
    {
        this.isOver = false;
    ...
    preRender(): void
    {
        const time = this.game.loop.now;
        const delta = this.game.loop.delta;
        const scenes = this.game.scene.getScenes(true, true);
     ...
     */
    setCursor(interactiveObject: any): void
    {
     ...
     */
    resetCursor(interactiveObject: any, forceReset?: boolean): void
    {
     ...
     */
    addPointer(quantity: number = 1): any[]
    {
        const output = [];
     ...
     */
    updateInputPlugins(type: number, pointers: any[]): void
    {
        const scenes = this.game.scene.getScenes(false, true);
        ...
        for (let i = 0; i < scenes.length; i++)
        {
            ...
            if (scene.sys.input)
            {
                const capture = scene.sys.input.update(type, pointers);
    ...
    onTouchStart(event: TouchEvent): void
    {
        ...
        for (let c = 0; c < event.changedTouches.length; c++)
        {
            const changedTouch = event.changedTouches[c];
        ...
        for (let c = 0; c < event.changedTouches.length; c++)
        {
            ...
            for (let i = 1; i < pointers.length; i++)
            {
                ...
                if (pointer.active && pointer.identifier === changedTouch.identifier)
                {
                    ...
                    const overCanvas = element === this.canvas;
    ...
    {
        const mousePointer = this.mousePointer;
    ...
    {
        const isLocked = this.mouse.locked;
     ...
     */
    inputCandidate(gameObject: any, camera: any): boolean
    {
     ...
     */
    hitTest(pointer: any, gameObjects: any[], camera: any, output?: any[]): any[]
    {
        ...
        const parentMatrix = this._tempMatrix2;
        ...
        for (let i = 0; i < gameObjects.length; i++)
        {
            ...
            const px = tempPoint.x + (csx * gameObject.scrollFactorX) - csx;
            const py = tempPoint.y + (csy * gameObject.scrollFactorY) - csy;
     ...
     */
    pointWithinHitArea(gameObject: any, x: number, y: number): boolean
    {
     ...
     * @return {boolean} `true` if the coordinates were inside the Game Objects hit area, otherwise `false`.
     */
    pointWithinInteractiveObject(object: any, x: number, y: number): boolean
    {
        if (!object.hitArea)
        {
            return false;
     ...
     */
    transformPointer(pointer: any, pageX: number, pageY: number, wasMove: boolean): void
    {
        ...
        const x = this.scaleManager.transformX(pageX);
        const y = this.scaleManager.transformY(pageY);
