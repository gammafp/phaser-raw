 ...
 */
export class Threshold extends Controller {
    edge1: number[];
    edge2: number[];
    invert: boolean[];

    constructor(camera: any, edge1?: number | number[], edge2?: number | number[], invert?: boolean | boolean[])
    {
     ...
     */
    setEdge(edge1?: number | number[], edge2?: number | number[]): this
    {
        if (edge1 === undefined)
        {
            edge1 = 0.5;
     ...
     * @return {Phaser.Filters.Threshold} This Threshold instance.
     */
    setInvert(invert?: boolean | boolean[]): this
    {
        if (invert === undefined)
        {
            invert = false;
