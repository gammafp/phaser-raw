...
export class BaseSound extends EventEmitter {
     ...
     */
    updateMarker(marker: any): boolean
    {
        if (!marker || !marker.name || typeof marker.name !== 'string')
        {
            return false;
     ...
     */
    removeMarker(markerName: string): any
    {
     ...
     */
    play(markerName?: string | any, config?: any): boolean
    {
    ...
    applyConfig(): void
    {
        (this as any).mute = this.currentConfig.mute;
        (this as any).volume = this.currentConfig.volume;
        (this as any).rate = this.currentConfig.rate;
        (this as any).detune = this.currentConfig.detune;
        (this as any).loop = this.currentConfig.loop;
        (this as any).pan = this.currentConfig.pan;
    }
