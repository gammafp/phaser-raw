...
const init = () =>
{
    ...
    else if ((/Chrome\/(\d+)/).test(ua) && !OS_DEVICE.windowsPhone)
    {
        Browser.chrome = true;
    ...
    else if ((/AppleWebKit\/(?!.*CriOS)/).test(ua) && OS_DEVICE.iOS)
    {
    ...
    else if ((/Version\/(\d+\.\d+(\.\d+)?) Safari/).test(ua) && !OS_DEVICE.windowsPhone)
    {
        Browser.safari = true;
...
export const BROWSER_DEVICE = init();
