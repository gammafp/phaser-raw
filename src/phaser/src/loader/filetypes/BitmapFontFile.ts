...
const ParseXMLBitmapFont = require('../../gameobjects/bitmaptext/ParseXMLBitmapFont');
 ...
 */
export class BitmapFontFile extends MultiFile {

    constructor(loader: any, key: string | Record<string, any>, textureURL?: string | string[], fontDataURL?: string, textureXhrSettings?: any, fontDataXhrSettings?: any)
    {
        let image: ImageFile;
        let data: XMLFile;
        ...
        if (image.linkFile)
        {
            ...
            super(loader, 'bitmapfont', key as string, [ image, data, image.linkFile ]);
        }
        else
        {
            super(loader, 'bitmapfont', key as string, [ image, data ]);
        }
    }
    ...
    addToCache(): void
    {
        if (this.isReadyToProcess())
        {
            ...
            const xml = this.files[1];

            image.addToCache();

            const texture = image.cache.get(image.key);

            const data = ParseXMLBitmapFont(xml.data, image.cache.getFrame(image.key), 0, 0, texture);
 ...
 */
FileTypesManager.register('bitmapFont', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, textureURL?: string | string[], fontDataURL?: string, textureXhrSettings?: any, fontDataXhrSettings?: any): any
{
    let multifile: BitmapFontFile;
