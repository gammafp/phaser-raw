 ...
 */
export class MultiAtlasFile extends MultiFile {

    constructor(loader: any, key: string | Record<string, any>, atlasURL?: string, path?: string, baseURL?: string, atlasXhrSettings?: any, textureXhrSettings?: any)
    {
        ...
        const data = new JSONFile(loader, key as string, atlasURL, atlasXhrSettings);

        super(loader, 'multiatlas', key as string, [ data ]);
    ...
    onFileComplete(file: any): void
    {
        ...
        if (index !== -1)
        {
            ...
            if (file.type === 'json' && file.data.hasOwnProperty('textures'))
            {
                ...
                const baseURL = GetFastValue(config, 'baseURL', this.baseURL);
                const path = GetFastValue(config, 'path', this.path);
                ...
                for (let i = 0; i < textures.length; i++)
                {
                    ...
                    const key = 'MA' + this.multiKeyIndex + '_' + textureURL;

                    const image = new ImageFile(loader, key, textureURL, textureXhrSettings);
                    ...
                    {
                        const normalMap = new ImageFile(loader, key, textures[i].normalMap, textureXhrSettings);
    ...
    addToCache(): void
    {
        if (this.isReadyToProcess())
        {
            ...
            for (let i = 1; i < this.files.length; i++)
            {
                ...
                for (let t = 0; t < fileJSON.data.textures.length; t++)
                {
                    const item = fileJSON.data.textures[t];
            ...
            const normalMapsOrUndefined = normalMaps.length === 0 ? undefined : normalMaps;

            this.loader.textureManager.addAtlasJSONArray(this.key, images, data, normalMapsOrUndefined);
 ...
 */
FileTypesManager.register('multiatlas', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, atlasURL?: string, path?: string, baseURL?: string, atlasXhrSettings?: any): any
{
    let multifile: MultiAtlasFile;
