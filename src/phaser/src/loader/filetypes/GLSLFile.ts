 ...
 */
export class GLSLFile extends File {

    constructor(loader: any, key: string | Record<string, any>, url?: string, shaderType: ShaderType = 'fragment', xhrSettings?: any)
    {
        let extension: string = 'glsl';
    ...
    addToCache(): void
    {
        ...
        let block = this.extractBlock(data, 0);

        if (block)
        {
            while (block)
            {
                const key = this.getShaderName(block.header);
                const shaderType = this.getShaderType(block.header);
                const uniforms = this.getShaderUniforms(block.header);
                const shaderSrc = block.shader;
    ...
    getShaderName(headerSource: string[]): string
    {
        ...
        {
            const line = headerSource[i].trim();
     ...
     */
    getShaderType(headerSource: string[]): ShaderType
    {
        for (let i = 0; i < headerSource.length; i++)
        {
            const line = headerSource[i].trim();
            ...
            {
                return line.substring(5).trim() as ShaderType;
            }
        }
     ...
     */
    getShaderUniforms(headerSource: string[]): Record<string, any>
    {
        ...
        for (let i = 0; i < headerSource.length; i++)
        {
            ...
            if (line.substring(0, 8) === 'uniform.')
            {
                ...
                if (pos)
                {
                    const key = line.substring(8, pos);
     ...
     */
    extractBlock(data: string[], offset: number): ShaderBlock | null
    {
        let headerStart = -1;
        let headerEnd = -1;
        let blockEnd = -1;
 ...
 */
FileTypesManager.register('glsl', function (this: any, key: string | Record<string, any> | Array<Record<string, any>>, url?: string, shaderType?: ShaderType, xhrSettings?: any): any
{
