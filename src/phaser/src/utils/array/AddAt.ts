 ...
 */
export const AddAt = <T>(array: T[], item: T | T[], index: number = 0, limit?: number, callback?: Function, context?: any): T | T[] | null =>
{
    ...
    if (limit! > 0 && itemLength > (limit! - array.length))
    {
        item.splice(limit! - array.length);

        itemLength = limit! - array.length;
    }
